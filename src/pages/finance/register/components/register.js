...
export default function Register() {
  ...
  useEffect(() => {
    if (stopFIrst.current === true) {
      ...
      if (
        registerData["userFullname"].trim() !== "" ||
        registerData["userEmail"].trim() !== "" ||
        registerData.userPassword.trim() !== ""
      ) {
        setError((c) => {
  ...
  const handleSubmit = (e) => {
    if (
      ...
      setError({
        ...errors,
        field_email_empty: "Please Fill Email",
        field_password_empty: "Please Fill Password",
        field_username_empty: "Please Fill Username",
      });
      e.preventDefault();
    } else {
          ...
          if (getRegister !== null) {
            getRegister.map(({ userEmail }) => {
              if (value === userEmail) {
                setError({
                  ...errors,
                  email_wrong: "This Email is already exist",
                });
              }
              return 0;
            });
    ...
    <div className="form_container">
      <form onSubmit={handleSubmit}>
        <div className="mb-3">
          ...
          />
          <div>
            <span className="error_msg ">{errors.field_username_empty}</span>
          </div>
        </div>
        <div className="mb-3">
          ...
          <div>
            <span className="error_msg">{errors.field_email_empty}</span>
          </div>
        </div>
        <div className="mb-3">
          ...
          <div>
            <span className="error_msg">{errors.field_password_empty}</span>
          </div>
        </div>
